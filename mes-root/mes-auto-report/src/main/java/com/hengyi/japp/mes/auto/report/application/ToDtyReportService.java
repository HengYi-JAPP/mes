...
@Slf4j
@Singleton
public class ToDtyReportService {
    private final QueryService queryService;

    @Inject
    private ToDtyReportService(MesAutoConfig mesAutoConfig, QueryService queryService) {
        this.queryService = queryService;
    }

    @SneakyThrows
    public Mono<ToDtyReport> generate(String workshopId, LocalDate startLd, LocalDate endLd) {
        ...
        final MongoCollection<Document> collection = Report.mongoCollection(SilkCarRecord.class);
        return Flux.fromArray(topDocs.scoreDocs)
                .map(scoreDoc -> Jlucene.toDocument(searcher, scoreDoc))
                .flatMap(it -> collection.find(eq(ID_COL, it.get("id"))))
                .collectList()
                .map(ToDtyReport::new);
    }
}
