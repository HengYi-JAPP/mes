 ...
 */
@Data
public class ManualPackageBoxesData {
    private final List<PackageBox> packageBoxes;
    private final ForkJoinTask<Collection<Item>> task;

    public ManualPackageBoxesData(List<PackageBox> packageBoxes) {
    ...
    private class SingleTask extends RecursiveTask<Collection<Item>> {
        ...
        protected Collection<Item> compute() {
            ...
            if (size == 1) {
                final List<AAReportSilkCarRecord> silkCarRecords = AAReport.packageBoxSilkCarRecords(packageBox);
                final Item item = new Item(packageBox, silkCarRecords);
    ...
    public class Item {
        ...
        private final List<AAReportSilkCarRecord> silkCarRecords;
        private Batch batch;
        private Grade grade;
        ...
        public Map<Triple<Batch, Line, SaleType>, Integer> toData() {
            ...
            silkCarRecords.parallelStream()
                    .flatMap(silkCarRecord -> silkCarRecord.getInitSilks().parallelStream()
                                    .map(SilkRuntime::getSilk)
//                            .filter(silk -> gradeAA.equals(silk.getGrade()))
                    )
