...
import {InitSuccess} from '../actions/permission-manage-page';
import {ofRouteChangePage} from './core.effects';

@Injectable()
export class PermissionManagePageEffects {
  @Effect() init$ = this.actions$.pipe(
    ofRouteChangePage(PermissionManagePageComponent),
    tap(() => this.store.dispatch(new SetLoading())),
    switchMap(() => {
      return this.apiService.listPermission()
        .pipe(
          map(permissions => new InitSuccess({permissions})),
